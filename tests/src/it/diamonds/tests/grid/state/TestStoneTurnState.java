...
public class TestStoneTurnState extends PlayFieldTestCase
{
    ...
    {
        AbstractControllerState newState = state.update(0, controller, new ScoreCalculator(0), new StoneCalculator());
        try
        {
            newState = (StoneTurnState)newState;
            fail();
        }
    ...
    public void testStateNotChangedWithMorphingGem()
    {
        ...
        AbstractControllerState newState = state.update(0, controller, null, null);
        assertNotNull((StoneTurnState)newState);
    }
