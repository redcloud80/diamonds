...
public class TestStoneFallState extends PlayFieldTestCase
{
    ...
    {
        assertTrue(state.update(environment.getTimer().getTime(), controller, null, new StoneCalculator()).isCurrentState("WaitBeforeNewGemsPair"));
    }
    ...
    public void testStateChangesWhenStoneDrops()
    {
        ...
        stone.drop();
        assertTrue(state.update(environment.getTimer().getTime(), controller, null, null).isCurrentState("WaitBeforeNewGemsPair"));
    }
    ...
    public void testStonesDuringInsertionWillInsertedNext()
    {
        ...
        assertEquals("there must be present only a stone", 1, controller.getGrid().getNumberOfDroppables());
    }
    ...
    private boolean isFirstRowFreeForFalling(GridController controller)
    {
        ...
        {
            if (!controller.getGrid().isCellFree(Cell.create(1, i)))
            {
                return false;
            }
        }
    ...
    public void testStoneFallFaster()
    {
        ...
        controller.update(environment.getTimer().getTime(), new ScoreCalculator(0));
