...
public class TestEvent extends TestCase
{
    ...
    public void testNoEqualEncodings()
    {
        ...
        for (State state : State.values())
        {
            ...
            {
                byte id = Event.create(code, state).encode();
    ...
    public void testEncodeAndDecodeEvents()
    {
        Event eventToEncode;
        Event decodedEvent;
        ...
        for (State state : State.values())
        {
            for (Code code : Code.values())
            {                
                eventToEncode = Event.create(code, state);
                
                decodedEvent = Event.create(eventToEncode.encode());
                assertEquals(eventToEncode.getCode(), decodedEvent.getCode());
                assertEquals(eventToEncode.getState(), decodedEvent.getState());
            }
        }
    }
}
