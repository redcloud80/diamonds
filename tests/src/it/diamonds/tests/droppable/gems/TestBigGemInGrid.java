...
public class TestBigGemInGrid extends GridTestCase
{
    ...
    public void test2x3BlockIsDetected()
    {
        ...
        assertEquals(1, getNumberOfExtensibleObject());
    }
    ...
    public void testGridIsUpdatedWithBigGems() throws IOException
    {
        ...
        GridController controller = new GridController(environment, new GridRenderer(environment, new Point(0, 0), grid), inputReactor, GemQueue.create(environment, new MockRandomGenerator(randomSequence)));

        GridController gridController = controller;

        grid = gridController.getGridRenderer().getGrid();

        gridController.update(environment.getTimer().getTime(), scoreCalculator, grid);
    ...
    public void testBigGemMoveDownABit()
    {
        ...
        float top = topGem.getPositionInGridLocalSpace().getY();
        float bigGemTop = bigGem.getPositionInGridLocalSpace().getY();
