...
public class BigGem extends AbstractDroppable
{
    ...
    private void updateSpritePosition(Direction direction)
    {
        ...
        {
            getPositionInGridLocalSpace().setY(getPositionInGridLocalSpace().getY()
                - Cell.SIZE_IN_PIXELS);
        }

        if (direction.equals(Direction.GO_LEFT))
        {
            getPositionInGridLocalSpace().setX(getPositionInGridLocalSpace().getX()
                - Cell.SIZE_IN_PIXELS);
        }
    }
    ...
    private MergeResult merge(Grid grid, Direction direction)
    {
        ...
        {
            getPositionInGridLocalSpace().setX(droppable.getPositionInGridLocalSpace().getX());
            getPositionInGridLocalSpace().setY(droppable.getPositionInGridLocalSpace().getY());
        }
