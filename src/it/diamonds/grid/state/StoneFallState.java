...
public class StoneFallState extends AbstractControllerState
{
    ...
    public AbstractControllerState update(long timer, GridController gridController, ScoreCalculator scoreCalculator, StoneCalculator stoneCalculator)
    {
        ...
        if (gridController.getGridRenderer().getGrid().isColumnFull(4))
        {
        ...
        if (areAllDropped(stones) && stonesToInsert == 0)
        {
            ...
            gridController.getGridRenderer().getGrid().setNormalGravity();
    ...
    {
        gridController.getGridRenderer().getGrid().setStrongestGravity();
        ...
        registerHeightOfColumns(gridController.getGridRenderer().getGrid());
    ...
    private void insertStones(GridController controller)
    {
        ...
        controller.getGridRenderer().getGrid().setStrongestGravity();

        for (int i = 0; i < stonesToInsert; i++)
        {
            int column = getLessFullColumn(controller.getGridRenderer().getGrid());
            ...
            if (isColumnFreeForFalling(controller, column))
            {
                ...
                controller.getGridRenderer().getGrid().insertDroppable(stone);
    ...
    {
        if (!controller.getGridRenderer().getGrid().isCellFree(Cell.create(0, column)))
        {
            return false;
        }

        Cell cell = Cell.create(1, column);
        if (!controller.getGridRenderer().getGrid().isCellFree(cell))
        {
            Droppable gem = controller.getGridRenderer().getGrid().getDroppableAt(cell);
            if (gem.isFalling())
            {
                return false;
            }
    ...
    private void makeAllDrop(GridController controller, DroppableList stones)
    {
        for (Droppable stone : stones)
        {
            controller.getGridRenderer().getGrid().updateDroppable(stone);
        }
    }
